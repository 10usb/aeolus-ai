...
function Finance::GetAvailableMoney(){
	return (AICompany.GetMaxLoanAmount() - AICompany.GetLoanAmount()) + AICompany.GetBankBalance(AICompany.COMPANY_SELF);
}
...
function Finance::Repay(){
	...
	if(AICompany.GetBankBalance(AICompany.COMPANY_SELF) < 15000) return false;
	
	local amount = floor(AICompany.GetBankBalance(AICompany.COMPANY_SELF) / 10000.0).tointeger() * 10000;
